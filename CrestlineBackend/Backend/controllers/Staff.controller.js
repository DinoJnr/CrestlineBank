...
const nodemailer = require("nodemailer"); // Standard Gmail Protocol Node
const Registeruser = require("../models/Registerusers.model");
const { Transaction, Activity } = require("../models/Transactivity");
const { sendInquiryResolutionEmail } = require("../utils/resolvedInquiry");
  ...
  try {
    ...
    if (!accountNumber || accountNumber.length !== 10) {
      return res
        ...
        .json({
          success: false,
          message: "Provide a valid 10-digit NUBAN account number.",
        });
    }

    const user = await Registeruser.findOne({ accountNumber }).select(
      "-password -transactionPin -cardPin -card.cvv",
    );
    ...
    if (!user) {
      return res
        ...
        .json({
          success: false,
          message: `No account found for ${accountNumber}`,
        });
    }
...
exports.enrollUser = async (req, res) => {
  try {
    ...
    const emailExists = await Registeruser.findOne({
    ...
    await logActivity(
      newUser._id,
      "ACCOUNT_CREATED",
      `New ${accountType || "personal"} account enrolled by staff.`,
    );
...
exports.updateUser = async (req, res) => {
  try {
    ...
    await logActivity(
      user._id,
      "PROFILE_UPDATED",
      `Fields updated by staff: ${Object.keys(updates).join(", ")}`,
    );
...
exports.updateUserStatus = async (req, res) => {
  try {
    ...
    if (currentComplianceTier !== undefined)
      updates.currentComplianceTier = Number(currentComplianceTier);
    ...
    if (Object.keys(updates).length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "No valid status/compliance fields provided.",
        });
    }
    ...
    return res
      ...
      .json({
        success: true,
        message: "Account compliance/status updated.",
        data: updated,
      });
  } catch (err) {
...
exports.updateUserLimits = async (req, res) => {
  try {
      ...
      if (req.body[key] !== undefined) {
        ...
        if (isNaN(val) || val < 0) {
          return res
            ...
            .json({
              success: false,
              message: `Invalid value for limit key: ${key}`,
            });
        }
    ...
    if (Object.keys(updates).length === 0) {
      return res
        .status(400)
        .json({ success: false, message: "No valid limit fields provided." });
    }
    ...
    const summary = Object.entries(updates)
      .map(
        ([k, v]) =>
          `${k.replace("transactionLimit.", "")}=₦${Number(v).toLocaleString()}`,
      )
      .join(", ");
    ...
    await logActivity(
      user._id,
      "LIMITS_UPDATED",
      `Transaction limits modified: ${summary}`,
    );
  ...
  try {
    ...
    if (user.status === "frozen") {
      return res
        ...
        .json({
          success: false,
          message: "Account is frozen — unfreeze before crediting.",
        });
    }
    ...
    await logActivity(
      user._id,
      "ACCOUNT_CREDITED",
      `₦${creditAmount.toLocaleString()} credited via ${channel || "Manual"}. Ref: ${reference}`,
    );
  ...
  try {
    ...
    if (user.status === "frozen") {
      return res
        ...
        .json({
          success: false,
          message: "Account is frozen — cannot debit a frozen account.",
        });
    }
    ...
    if (user.pndType === "Debit" || user.pndType === "Full") {
      return res
        ...
        .json({
          success: false,
          message: `Post-No-Debit (${user.pndType}) is active on this account.`,
        });
    }
    ...
    await logActivity(
      user._id,
      "ACCOUNT_DEBITED",
      `₦${debitAmount.toLocaleString()} debited via ${channel || "Manual"}. Ref: ${reference}`,
    );
...
exports.reverseTransaction = async (req, res) => {
  try {
    ...
    await logActivity(
      ...
      `Original ref: ${txn.reference} | Reversal ref: ${reversalRef} | Reason: ${reason || "N/A"}`,
    );
...
exports.blockCard = async (req, res) => {
  try {
    ...
    if (!user.card || user.cardStatus === "none") {
      return res
        .status(400)
        .json({ success: false, message: "No card found on this account." });
    }
...
exports.unblockCard = async (req, res) => {
  try {
    ...
    if (user.cardStatus === "hotlisted") {
      return res
        ...
        .json({
          success: false,
          message:
            "Hotlisted cards cannot be unblocked — issue a replacement card.",
        });
    }
    ...
    if (user.cardStatus === "active") {
      return res
        .status(409)
        .json({ success: false, message: "Card is already active." });
    }
...
exports.replaceCard = async (req, res) => {
  try {
    ...
    await logActivity(
      ...
      `Reason: ${reason || "N/A"} | Delivery: ${deliveryAddress || user.address}`,
    );
...
exports.resetPin = async (req, res) => {
  try {
    ...
    await logActivity(
      user._id,
      "PIN_RESET",
      "Card PIN reset by staff — OTP dispatched to registered phone.",
    );
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    await logActivity(
      ...
      reason || "Password reset initiated by staff.",
    );
...
exports.generateStatement = async (req, res) => {
  try {
    ...
    await logActivity(
      user._id,
      "STATEMENT_GENERATED",
      `E-statement generated by staff. Period: ${from || "all"} – ${to || "now"}`,
    );
...
exports.notifyUser = async (req, res) => {
  try {
    ...
    if (!channel || !message) {
      return res
        .status(400)
        .json({ success: false, message: "Channel and message are required." });
    }
    ...
    await logActivity(
      user._id,
      `NOTIFICATION_SENT_${channel.toUpperCase()}`,
      `Subject: ${subject || "N/A"} | Message: ${message.substring(0, 80)}...`,
    );

    const channelLabel =
      { sms: "SMS", email: "email", push: "push notification" }[channel] ||
      channel;
...
exports.addMandate = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message:
            "Beneficiary, account, amount, frequency and start date are required.",
        });
    }
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message:
            "Cannot create a standing order on a frozen, blacklisted, or fully restricted account.",
        });
    }
    ...
    await logActivity(
      user._id,
      "MANDATE_CREATED",
      `Standing order to ${beneficiary} (${beneficiaryAccount}) — ₦${Number(amount).toLocaleString()} ${frequency}`,
    );
...
exports.updateMandateStatus = async (req, res) => {
  try {
    ...
    if (!mandate)
      return res
        .status(404)
        .json({ success: false, message: "Mandate not found." });
    ...
    return res
      ...
      .json({
        success: true,
        message: `Standing order ${status.toLowerCase()}.`,
        data: mandate,
      });
  } catch (err) {
...
exports.cancelMandate = async (req, res) => {
  try {
    ...
    await logActivity(
      user._id,
      "MANDATE_CANCELLED",
      `Mandate ${mandateId} cancelled`,
    );

    return res
      .status(200)
      .json({ success: true, message: "Standing order cancelled." });
  } catch (err) {
  ...
  try {
    ...
    if (!name || !bvn) {
      return res
        ...
        .json({
          success: false,
          message: "Signatory name and BVN are required.",
        });
    }
    ...
    if (bvn.length !== 11) {
      return res
        .status(400)
        .json({ success: false, message: "BVN must be 11 digits." });
    }
    ...
    if (
      ...
      return res
        ...
        .json({
          success: false,
          message:
            "Cannot add a signatory on a frozen, blacklisted, or fully restricted account.",
        });
    }
    ...
    return res
      ...
      .json({
        success: true,
        message: "Signatory added — mandate updated.",
        data: signatory,
      });
  } catch (err) {
...
exports.removeSignatory = async (req, res) => {
  try {
    ...
    const target = user.signatories.find(
      (s) => s._id.toString() === signatoryId,
    );
    if (!target)
      return res
        .status(404)
        .json({ success: false, message: "Signatory not found." });
    ...
    if (target.status === "Active" && remainingActive.length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "Cannot remove the last active signatory on this account.",
        });
    }

    user.signatories = user.signatories.filter(
      (s) => s._id.toString() !== signatoryId,
    );
    ...
    await logActivity(
      user._id,
      "SIGNATORY_REMOVED",
      `Signatory ${signatoryId} removed`,
    );

    return res
      .status(200)
      .json({ success: true, message: "Signatory removed — mandate updated." });
  } catch (err) {
...
exports.getPending = async (req, res) => {
  try {
    const inquiries = await PendingInquiry.find()
      .populate("userId", "fullName email accountNumber phone")
      .sort({ createdAt: -1 });
...
exports.getArchived = async (req, res) => {
  try {
    const inquiries = await ArchivedInquiry.find()
      .populate("userId", "fullName email accountNumber phone")
...
exports.resolveInquiry = async (req, res) => {
  try {
    ...
    if (!resolutionNotes?.trim()) {
      return res
        .status(400)
        .json({ success: false, message: "Resolution notes are required." });
    }

    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res
        .status(400)
        .json({ success: false, message: "Invalid inquiry ID." });
    }

    const inquiry = await PendingInquiry.findById(id).populate(
      "userId",
    ...
    await ArchivedInquiry.create({
      userId: inquiry.userId._id || inquiry.userId,
      subject: inquiry.subject,
      message: inquiry.message,
      originalTicketId: inquiry._id,
    ...
    if (inquiry.userId) {
      await sendInquiryResolutionEmail(inquiry.userId, {
        subject: inquiry.subject,
        message: inquiry.message,
        resolutionNotes: trimmedNotes,
      });
    }
    await PendingInquiry.findByIdAndDelete(id);

    return res.status(200).json({
      success: true,
      message: "Inquiry resolved and archived. Customer notification queued.",
    });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    const pendingKycCount = await Registeruser.countDocuments({ "kyc.status": "Pending" });
    const balanceMetrics = await Registeruser.aggregate([
      {
        $group: {
          _id: null,
          totalDeposits: { $sum: "$balance" },
          totalLoans: { $sum: "$activeLoan.balance" }
        }
      }
    ]);

  
    const rawDeposits = balanceMetrics[0]?.totalDeposits || 0;
    const rawLoans = balanceMetrics[0]?.totalLoans || 0;
    ...
    return res.status(200).json({
      success: true,
      data: {
        activeUsers: { val: activeUsersCount.toLocaleString(), growth: "+12%" },
        totalDeposits: { val: formatCurrency(rawDeposits), growth: "+5.4%" },
        loanVolume: { val: formatCurrency(rawLoans), growth: "+2.1%" },
        pendingKyc: { val: pendingKycCount.toString(), growth: "High Priority" }
      }
    });
  } catch (err) {
    console.error("❌ Stats Aggregation Internal Error:", err.message);
    return res.status(500).json({ 
      success: false, 
      message: "Could not aggregate core operational variables.",
      error: err.message 
    });
  }
};
...
exports.getPendingInquiries = async (req, res) => {
  try {
    ...
    const queue = await PendingInquiry.find()
      .populate("userId", "fullName email phone accountNumber")
      .sort({ createdAt: -1 });
    ...
    console.error("❌ Queue Processing Failure:", err.message);
    return res.status(500).json({ 
      success: false, 
      message: "Internal server error fetching real-time inquiry streams." 
    });
  }
};